...
public class Main extends Application {
	...
	/* Make pane */
	/*Invokes the makeCritter functions;
	  concerned about the type and number of Critter*/
	...
	/* Step pane */
	/*Invokes the doTimeStep functions;
	  concerned about the interaction between Critters*/
	...
	/* Animate pane */
	/* The Handle now does Critter worldTimeSteps, same thing as click worldTimeStep bunch of times */
	...
	private static void sceneConfig() {
		scene.setHgap(30);
		scene.setPadding(new Insets(10, 20, 20 ,20));
	...
	private static void makeAmtConfig() {
		...
		makeAmtMult.setMinorTickCount(5);
		makeAmtMult.setPrefWidth(2000);
		makeAmtMult.valueProperty().addListener(new ChangeListener<Number>() {
			public void changed(ObservableValue<? extends Number> ov, Number old_val, Number new_val) {
				if (((int) makeAmtSlider.getValue() * new_val.intValue() < 1)) {
				    makeButton.setDisable(true);
				}
				else {
				    makeButton.setDisable(false);
	...
	private static void handlerInit() {
        ...
        /* Step button handler */
        /* Calls the worldTimeStep function however many times */
	...
	/**
	 * Toggles pane enable/disable when animating, disable everything when animating
	 */
